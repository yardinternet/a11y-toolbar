...
export const addDeepLButton = (
	...
	const init = (): void => {
		...
		const savedLanguage =
			window.sessionStorage.getItem( 'DeepLSelectedLanguage' ) ?? '';
	...
	const createLanguageModalContent = (): HTMLElement => {
		...
		description.classList.add(
			'a11y-toolbar__translate-description-deepl'
		);
	...
	const applyTranslations = (
		translations: Array< { text: string; translation: string } >
	): void => {
	...
	const replaceTextPreservingEdgeWhitespace = (
		...
		const nextElement =
			node.nextSibling instanceof HTMLElement ? node.nextSibling : null;
		if (
			! trailingWhitespace &&
